...
class FHXDetailViewController: UIViewController, UIContextMenuInteractionDelegate {
    ...
    private func buildUI() {
        ...
        // 普通长文本使用 UITextView 展示，补充与日志列表一致的长按复制菜单。
    ...
    /// 复制详情时始终使用完整原始日志，不受 JSON 当前折叠状态影响。
    private func copyText() -> String? {
        ...
        return "\(model.file).\(model.function):[\(model.line)]\n\(model.message)"
    }

    private func copyAllDetails() {

        guard let text = copyText() else {
            return
        }
...
extension FHXDetailViewController {
    ...
    func contextMenuInteraction(
        ...
        guard interaction.view === textView,
              copyText() != nil else {
            return nil
        }
